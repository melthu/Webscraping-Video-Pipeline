"""
Updated test suite for all scrapers with consistent mocking and error handling.
"""
...
from tests.mock_responses import MOCK_RESPONSES
...
class TestBaseScraperFunctionality(BaseTestCase):
    """Test cases for the base scraper functionality using a concrete implementation."""
    
    def setUp(self):
        """Set up test environment."""
        super().setUp()
        self.config = {"test_config": True}
        # Use a concrete implementation instead of the abstract base class
        self.scraper = PexelsScraper(self.config)
    
    def test_make_request_success(self):
        """Test the _make_request method with successful response."""
        ...
        with patch('requests.Session.get') as mock_get:
            ...
            # Call the method directly to ensure it's actually called
    ...
    def test_make_request_http_error(self):
        """Test the _make_request method with HTTP error."""
        # Setup mock response for failed request with HTTP error
        with patch('requests.Session.get') as mock_get:
            mock_response_fail = MagicMock()
            mock_response_fail.status_code = 404
            # Set up raise_for_status to raise an exception
            mock_response_fail.raise_for_status.side_effect = requests.exceptions.HTTPError("404 Client Error")
    ...
    def test_make_request_exception(self):
        """Test the _make_request method with general exception."""
        # Setup mock for exception during request
        with patch('requests.Session.get', side_effect=Exception("Test exception")):
            # The method should catch the exception and return None
            response = self.scraper._make_request("https://example.com")
            self.assertIsNone(response)


class TestPexelsScraper(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.patches.append(self.env_patcher)
        ...
        self.patches.append(self.make_request_patcher)
...
class TestPixabayScraper(BaseTestCase):
...
class TestVidevoScraper(BaseTestCase):
...
class TestNASAScraper(BaseTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.config = {
            "search_url": "https://images-api.nasa.gov/search",  # Changed from base_url to search_url
            "per_page": 10,
            "request_delay": 0.01  # Fast for testing
        }
        ...
        self.patches.append(self.get_asset_info_patcher)
    
    def test_initialization(self):
        """Test scraper initialization."""
        self.assertEqual(self.scraper.name, "nasa")
        # Check search_url instead of base_url
...
class TestInternetArchiveScraper(BaseTestCase):
...
class TestWikimediaScraper(BaseTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.config = {
            "search_url": "https://commons.wikimedia.org/w/api.php",  # Use search_url instead of api_url
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.scraper.search_url, self.config["search_url"])  # Check search_url
        self.assertEqual(self.scraper.file_url, self.config["file_url"])  # Check file_url
        self.assertEqual(self.scraper.per_page, self.config["per_page"])
...
class TestCoverrScraper(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.patches.append(self.selenium_patcher)
        ...
        self.patches.append(self.close_selenium_patcher)
...
class TestNOAAScraper(BaseTestCase):
    ...
    def setUp(self):
        ...
        # Mock the _get_video_details method (correct method name)
        self.get_video_details_patcher = patch.object(self.scraper, '_get_video_details')
        self.mock_get_video_details = self.get_video_details_patcher.start()
        self.patches.append(self.get_video_details_patcher)
